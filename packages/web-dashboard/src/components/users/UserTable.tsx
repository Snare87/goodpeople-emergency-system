...
const UserTable: React.FC<UserTableProps> = ({ users, onStatusUpdate, onPermissionsUpdate, isAdmin }) => {
  if (users.length === 0) {
    return (
  ...
  const getRankBadge = (rank?: string) => {
    const actualRank = rank || '소방사';
    const colorClass = RANK_COLORS[actualRank] || 'bg-gray-100 text-gray-800';
    return <span className={`px-2 py-1 ${colorClass} rounded-md text-sm font-medium`}>{actualRank}</span>;
  };
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {users.map((user) => (
            <tr key={user.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div>
                  ...
                  <div className="mt-1 flex gap-2">
                    ...
                    <Badge variant={(POSITION_BADGE_VARIANTS[user.position || ''] || 'default') as BadgeVariant}>
                      {user.position}
                    </Badge>
                  </div>
                </div>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <Badge variant={STATUS_BADGE_VARIANTS[user.status] as BadgeVariant}>
